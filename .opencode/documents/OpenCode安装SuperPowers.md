...
```powershell
New-Item -ItemType SymbolicLink -Path "~/.config/opencode/plugins/superpowers.js" -Target "~/.config/opencode/superpowers/.opencode/plugins/superpowers.js" 

New-Item -ItemType SymbolicLink -Path "~/.config/opencode/skills/superpowers" -Target "/.config/opencode/superpowers/skills" 
```
...
如果 OpenCode 正在运行，先退出再重新启动。
重启后，Superpowers 会自动注入上下文，无需额外配置。

## 6、Superpowers 技能详细介绍
...
- 根本原因调查: 仔细阅读错误信息、一致地重现、检查最近更改、收集证据、追踪数据流
- 模式分析: 找到工作示例、与参考比较、识别差异、理解依赖
- 假设与测试: 形成单一假设、最小化测试、验证后继续
- 实现: 创建失败测试用例、实现单一修复、验证修复

**关键原则**: 如果3次修复都失败，停止并质疑架构。
...
### 7. using-git-worktrees (使用 Git Worktrees)

**用途**: 开始需要与当前工作区隔离的功能开发时，或在执行实现计划之前使用。

**核心功能**: Git worktrees 创建共享同一仓库的隔离工作区，允许同时在多个分支上工作而无需切换。
