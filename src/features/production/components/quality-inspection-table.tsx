...
import { useQualityInspectionList } from '@/queries/production'
...
import {
  ...
  qualityInspectionColumns,
} from './quality-inspection-columns'

const route = getRouteApi('/_authenticated/production/qc')
...
export function QualityInspectionTable({
  ...
  const { data: response, isLoading } = useQualityInspectionList({
    params: {
  ...
  const columns = qualityInspectionColumns({ onView, onDelete })
